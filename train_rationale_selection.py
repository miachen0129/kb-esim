...
from claim_verification.utils.Datasets import SciFactRationaleSelectionDataset
...
def build_word_ls(args):
    ...
    # --------------------Read Data--------------------
    trainset = SciFactRationaleSelectionDataset(args.corpus_path, args.train_path)
    devset = SciFactRationaleSelectionDataset(args.corpus_path, args.dev_path)
    ...
    with open('../../brain/scifact_word_ls.tsv','w+') as f:
        f.write("word\tcount\n")
...
def train(args):
    ...
    for epoch in range(1, args.epochs_num + 1):
        ...
        save_model(model, f"../../outputs/rationale/checkpoint_*{epoch}.bin")
    ...
    print('Final evaluation on the dev dataset')
    if torch.cuda.device_count() > 1:
...
if __name__ == '__main__':
    ...
    # Path options.
    parser.add_argument("--pretrained_model_path", type=str, default="models/google_model.bin", help="Path of the pretrained model.")
    parser.add_argument("--output_model_path", default="outputs/rationale", type=str,
                        help="Path of the output model.")
    parser.add_argument("--vocab_path", default="models/google_vocab.txt", type=str,
                        help="Path of the vocabulary file.")

    parser.add_argument("--train_path", type=str, default="datasets/meta/sciFact/claims_train.jsonl")
    parser.add_argument("--dev_path", type=str, default="datasets/meta/sciFact/claims_dev.jsonl")
    parser.add_argument("--corpus_path", type=str, default="datasets/meta/sciFact/corpus.jsonl")
    ...
    for i in range(len(devset)):
        if trainset[i]['label'] not in labels_cnt:
            labels_cnt[trainset[i]['label']] = 1
        else:
            labels_cnt[trainset[i]['label']] += 1

...
def merge_dataset(dataset_path, workers_num, instances_buffer_size):
    ...
    for i in range(workers_num):
        tmp_dataset_reader = open("datasets-tmp-"+str(i)+".pt", "rb")
        while True:
            try:
        ...
        os.remove("datasets-tmp-"+str(i)+".pt")
    f_writer.close()
...
class BertDataset(object):
    """
    Construct datasets for MLM and NSP tasks from the given corpus.
    ...
    def build_and_save(self, workers_num):
        """
        Build datasets from the given corpus.
    ...
    def worker(self, proc_id, start, end):
        print("Worker %d is building datasets ... " % proc_id)
        set_seed(self.seed)
        docs_buffer = []
        document = []
        pos = 0
        f_write = open("datasets-tmp-" + str(proc_id) + ".pt", "wb")
...
class BertDataLoader(object):
    ...
    def __init__(self, args, dataset_path, batch_size, proc_id, proc_num, shuffle=False):
        ...
        # We only need to read datasets once when buffer is big enough to load entire datasets.

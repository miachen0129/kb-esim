...
def claim_verification():
    ...
    for i, (claim_label, ids_batch, premises_batch, hypotheses_batch, sentences_batch, labels_batch, label_ids_batch) in \
        ...
        # class_cnt = torch.bincount(avg_pred, minlength=3)
        ...
        # final_predict = torch.argmax(weighted_probs)

        # # method2
        # avg_pred = torch.argmax(avg_prob, dim=1)
        # class_cnt = torch.bincount(avg_pred, minlength=3)
        # print(class_cnt)
        # if class_cnt[0] > class_cnt[1] and class_cnt[0] > class_cnt[2]:
        #     final_predict = 0
        # elif class_cnt[2] > class_cnt[0] and class_cnt[2] > class_cnt[1]:
        #     final_predict = 2
        # else:
        #     final_predict = 1
